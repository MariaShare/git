...
Не забудь после этого перейти в папку с репозиторем в терминале: `cd git-rules`.
...
4. Открой Git Graph (кнопка `Git Graph` в нижней панели VS Code). Убедись, что в нем есть коммит с названием `root`.
...
5. Через VS Code создай файл `init.md` со следующим содержимым: 
```
## S1. Все локально
...
6. Выполни команду `git status`. В секции `Untracked files` должен появиться `init.md` (и подсвечен красным цветом).  
...
7. Выполни команду `git add init.md`. Теперь `init.md` находится в Commit Index.

8. Сделай коммит с сообщением `Add init.md`. Для этого выполни команду  `git commit -m "Add init.md"`.

9. Открой Git Graph и убедись, что коммит появился в истории коммитов
...
11. Выполни `git status` и убедись что `commit.md` находится в секции `Untracked files` (и подсвечен красным цветом)

*Выглядеть должно так:*  
<img src="https://raw.githubusercontent.com/kontur-courses/git/master/images/cli-images/status-untracked.png">

12. Добавь `commit.md` в Commit Index: `git add commit.md`

13. Выполни `git status` и убедись что `commit.md` находится в секции `Changes to be commited` (и подсвечен зеленым цветом)
...
15. Выполни `git status`. Убедись, что `commit.md` находится как в Working Directory (секция `Changes not staged for commit`), так и в Commit Index (секция `Changes to be commited`)

16. Открой в VS Code вкладку Source Control (`Ctrl + Shift + G`) и убедись, что `commit.md` находится как в верхней части окна, так и в нижней,
...
17. Выполни коммит с сообщением `Add commit.md header` (можешь посмотреть пункт 8, чтобы вспомнить как это делать)

18. В истории коммитов (Git Graph) найди только что созданный коммит и убедись, что в него попали только изменения из Commit Index

19. Закоммить оставшиеся в Working directory изменения с сообщением `Change commit.md`
Не забудь перед коммитом добавить оставшиеся изменения в Commit Index, иначе коммит будет сделан (команда `git add .`)!
...
2. Найди в Git Graph начальный коммит `root`, выбери его и с помощью контекстного меню выполни `Checkout...`
...
10. Перейди назад на ветку `master`
...
Но можно воспользоваться `Amend Commit`. Эта команда позволяет дополнить последний коммит дополнительными изменениями: `git commit --amend`
...
*Теперь история коммитов должна выглядеть так:*  
<img src="https://raw.githubusercontent.com/kontur-courses/git/master/images/cli-steps/09-before.png">
...
7. Удали ветку `sheet-feature`. Это придется сделать с force, потому что эта ветка связана
...
5. Добавь изменения в Commit Index и закоммить их с опцией `--amend` (Amend commit), чтобы не создавать лишний коммит.
...
1. Найди в Git Graph начальный коммит `root`, выбери его и с помощью контекстного меню выполни
